...
try {
    // Fetch user details
    $stmt = $pdo->prepare("SELECT name, email, phone, year, course, completion_year, ministry, password FROM members WHERE id = ?");
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update_profile'])) {
        ...
        // Validate inputs
        if (empty($name) || empty($email) || empty($phone) || empty($year) || empty($course) || empty($completion_year) || empty($ministry)) {
        ...
        // Update user details
        $stmt = $pdo->prepare("UPDATE members SET name = ?, email = ?, phone = ?, year = ?, course = ?, completion_year = ?, ministry = ? WHERE id = ?");
        $stmt->execute([$name, $email, $phone, $year, $course, $completion_year, $ministry, $user_id]);
            ...
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background-color: #f4f7fa; border-radius: 10px;">
                ...
                <div style="padding: 20px; background-color: #ffffff; border-radius: 0 0 10px 10px;">
                    ...
                    <ul style="list-style: none; padding: 0; color: #333333; font-size: 15px;">
                        ...
                        <li style="margin-bottom: 10px;"><strong style="color: #0207ba;">Completion Year:</strong> ' . htmlspecialchars($completion_year) . '</li>
                        <li style="margin-bottom: 10px;"><strong style="color: #0207ba;">Ministry:</strong> ' . htmlspecialchars($ministry) . '</li>
                    </ul>
...
<body>
        ...
        <div class="row g-4">
            ...
            <div class="col-md-4 text-center">
                <div class="card p-4">
                    ...
                    <p class="text-muted"><?php echo htmlspecialchars($user['ministry'] ?? 'No Ministry'); ?></p>
                </div>
            </div>
                ...
                <div class="card p-4">
                    ...
                    <form action="profile.php" method="POST" enctype="multipart/form-data" id="profileForm">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="completion_year" id="completion_year" class="form-control" value="<?php echo htmlspecialchars($user['completion_year']); ?>" required>
                        </div>
                        <div class="mb-3">
                            <label for="ministry" class="form-label">Ministry</label>
                            <input type="text" name="ministry" id="ministry" class="form-control" value="<?php echo htmlspecialchars($user['ministry']); ?>" required>
                        </div>
                ...
                <div class="card p-4 mt-4">
                    ...
                    <div class="card p-3 mt-4">
                        ...
                        <div class="input-group mb-3">
                            <input type="text" id="current_password_display" class="form-control" readonly value="<?php echo htmlspecialchars($user['password'] ? '********' : 'Not Applicable'); ?>" style="border-radius: 8px;">
                            <span class="password-toggle" onclick="togglePasswordDisplay('current_password_display', this)">Show</span>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        function togglePasswordDisplay(inputId, toggleElement) {
            const input = document.getElementById(inputId);
            if (input.value === 'Not Applicable') return; // No toggle if not applicable
            if (input.type === 'text') {
                input.type = 'password';
                input.value = '********';
                toggleElement.textContent = 'Show';
                ...
                input.type = 'text';
                input.value = '<?php echo htmlspecialchars($user['password'] ?? ''); ?>'; // Show actual password if set
                toggleElement.textContent = 'Hide';
            }
        }
