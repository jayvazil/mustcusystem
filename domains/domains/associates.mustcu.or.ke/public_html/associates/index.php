...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['login'])) {
    if ($login_attempts >= 5) {
        ...
        if (empty($email) || empty($password)) {
            ...
            $stmt = $pdo->prepare("SELECT id, password, phone FROM associates WHERE email = ? LIMIT 1");
            $stmt->execute([$email]);
            $user = $stmt->fetch();
            error_log("Database query result: " . json_encode($user));

            if ($user && ($user['password'] === $password || ($user['password'] === null && $user['phone'] === $password))) {
                $_SESSION['user_id'] = $user['id'];
                $_SESSION['role'] = 'associate';
                $_SESSION['force_reset'] = ($user['password'] === null);
                $_SESSION['login_attempts'] = 0;
                header('Location: ' . ($_SESSION['force_reset'] ? 'reset_password.php' : 'dashboard.php'));
                exit;
            } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['reset_request'])) {
    ...
    if (empty($email)) {
        ...
        if (count($roles) === 0) {
            ...
            header('Location: reset_password.php?select_role=1');
            exit;
        }
    }
}
...
<head>
    ...
    <title>Associate Login - MUST CU</title>
    ...
    <style>
        ...
        body {
            background: linear-gradient(135deg, var(--primary-blue), #1e3c72 100%);
            min-height: 100vh;
...
<body>
    ...
    <div class="login-container">
        ...
        <?php endif; ?>
        <h2 class="text-center mb-4"><i class="fas fa-sign-in-alt"></i> Associate Login</h2>
        ...
        <div class="modal fade" id="resetModal" tabindex="-1">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header" style="background: linear-gradient(90deg, var(--primary-blue), #1b0aa2); color: #fff000;">
